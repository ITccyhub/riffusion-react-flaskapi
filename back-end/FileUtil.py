...
def fileUploads(file, is_public, user_id):
    try:
        # 先将文件保存到临时文件夹
        upload_dir = fileGetDir(0)
        if not os.path.exists(upload_dir):
        ...
        # 插入数据库文件地址、是否公开和上传用户 ID
        # 假设你的数据库操作函数为 insertFile 方法，接受文件地址、是否公开和用户 ID 参数


        return  json.dumps(DBUtil.insertFileRecord(file_path, is_public, user_id))
    except Exception as e:
        print(str(e))
        return json.dumps({'code': -1, 'message': str(e)})
